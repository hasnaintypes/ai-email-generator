...
import { ArrowLeft, Edit, Menu, Settings, Save } from "lucide-react"
...
export default function EditorPage() {
  ...
  const handleElementUpdate = (updatedElement) => {
    setEmailElements(emailElements.map((el) => (el.id === updatedElement.id ? updatedElement : el)))
    ...
    <div className="flex flex-col h-screen bg-muted/30">
      ...
      <header className="border-b bg-background p-4 flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 w-full md:w-auto justify-end">
          ...
          {isMobile && (
            <Button variant="outline" size="icon" onClick={() => setOpenSheet("elements")} className="md:hidden">
          ...
          <div className="flex flex-col md:flex-row flex-1 overflow-hidden">
              ...
              <div className="p-4">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {elements.map((element) => (
                    <ElementItem key={`element-${element.type}`} element={element} id={`element-${element.type}`} />
                  ))}
                </div>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto bg-muted/50 p-6 flex justify-center custom-scrollbar">
              <div className={`bg-white shadow-sm border w-[600px] min-h-[600px]`}>
                <SortableContext items={emailElements.map((el) => el.id)} strategy={verticalListSortingStrategy}>
                  <DroppableArea
                ...
                {emailElements.length === 0 && (
                  <div className="flex flex-col items-center justify-center h-[400px] text-center p-6 border-2 border-dashed m-4 rounded-md">
                    ...
                    <p className="text-sm text-muted-foreground max-w-xs">
                      Drag and drop elements from the sidebar to start building your email template
                    </p>
                  </div>
                )}
              </div>
            ...
            {selectedElement && (
              <div className="w-full md:w-72 border-l bg-background overflow-y-auto hidden md:block custom-scrollbar">
                ...
                <PropertyPanel element={selectedElement} onUpdate={handleElementUpdate} />
              </div>
            )}
      ...
      {isMobile && (
        <div className="fixed bottom-0 left-0 right-0 bg-background border-t p-2 flex justify-around">
          <Button
            ...
            size="sm"
            onClick={() => setOpenSheet(openSheet === "elements" ? "" : "elements")}
          ...
          <Button
            ...
            size="sm"
            onClick={() => setOpenSheet(openSheet === "settings" ? "" : "settings")}
            className="flex-1 mx-1"
      ...
      <Sheet open={openSheet === "elements"} onOpenChange={(open) => !open && setOpenSheet("")}>
      ...
      <Sheet open={openSheet === "settings"} onOpenChange={(open) => !open && setOpenSheet("")}>
        <SheetContent side="right" className="w-full sm:max-w-md p-0">
          <SheetHeader className="p-4 border-b">
            <SheetTitle>Element Settings</SheetTitle>
          </SheetHeader>
          <div className="overflow-y-auto h-full pb-20 custom-scrollbar">
            {selectedElement && <PropertyPanel element={selectedElement} onUpdate={handleElementUpdate} />}
          </div>
        </SheetContent>
      </Sheet>
    </div>
